...
public class DNSTest extends BaseDNSTest {
	...
	public void dnsTest() {
		...
		Assert.assertThat("Цена на товар " + playstation.getName() + " отображается неверно",
								cartPage.getPriceOfProduct(cartPage.getProductInCart(playstation)),
								Is.is(playstation.getPrice()));
		Assert.assertThat("Цена на товар " + detroit.getName() + " отображается неверно",
								cartPage.getPriceOfProduct(cartPage.getProductInCart(detroit)),
		...
		Assert.assertFalse("Товар отображается в корзине после удаления ",cartPage.checkProduct(detroit));
		Assert.assertTrue("Цена не уменьшилась на стоимость " + detroit.getName(),cartPage.wasPriseChangedInCart(detroit,basePage));
		...
		//18) нажать вернуть удаленный товар
		cartPage.restoreProduct();
		//19) проверить что 3 playstation снова в корзине и выбрана гарантия 24 месяца
		cartPage.waitUntilClick();
		Assert.assertTrue("Товар " + playstation.getName() + " не отображается в корзине ",cartPage.checkProduct(playstation));
